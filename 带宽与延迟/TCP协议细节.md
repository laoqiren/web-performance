...
再谈及重传时，需要先介绍几种实现可靠传输协议（停等协议、回退N协议、选择重传协议）中的选择重传协议，TCP使用选择重传协议。
### 选择重传协议
...
### 超时重传
上图所示的重传是超时重传，但是这可能需要等比较长的时间。比如`client`发送了`0,1,2,3`包，`server`端收到`0,1,3`即2号包丢失，返回`ACK`为`1,2,4`，2号包将一直等到超时才会重传。见上图。
